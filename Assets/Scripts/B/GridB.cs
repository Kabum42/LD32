	...
	void CreateGrid() {
		...
		for (int x = 0; x < gridSizeX; x++) {
			...
			for (int y = 0; y < gridSizeY; y++) {
				...
				grid[x,y] = new NodeB(height, worldPoint, x, y);
				//debugGrid += grid[x,y].height + ", ";
			}
			//Debug.Log (debugGrid);
		}
	}
		...
		for (int x = -1; x <= 1; x++){
			for (int y = -1; y <= 1; y++){
				...
				if(checkX >= 0 && checkX < gridSizeX && checkY >= 0 && checkY < gridSizeY){
					if(grid[checkX, checkY].height - node.height <= 1)
	...
	void OnDrawGizmos(){
		...
		if (displayGridGizmos){
			if (onlyDisplayPathGizmos){
				...
				if (grid != null) {
					NodeB playerNode = NodeFromWorldPoint(player.position);
